...
export function useExplorerDragAndDrop({
  ...
  const applyMoveResult = useCallback(
    (targetDir: string) => {
      ...
      const nextSelectedPaths = [...result.moves.map((move) => move.to), ...result.skipped];
      const latestMovedPath =
        result.moves[result.moves.length - 1]?.to ??
  ...
  const onDragStartNode = useCallback(
    (event: ReactDragEvent, node: FSNode) => {
