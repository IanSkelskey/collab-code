    ...
    >
      {openTabs.map((path) => {
        ...
        return (
          ...
          >
            {/* Active indicator */}
            {isActive && <div className="absolute top-0 left-0 right-0 h-[2px] bg-emerald-400" />}

            {/* File icon dot */}
            <span
              className={`w-2 h-2 rounded-full shrink-0 ${getIconColor(name)}`}
      ...
      {ctxMenu && (
        ...
        >
          <button
            ...
            onClick={() => fs.closeTab(ctxMenu.path)}
          >
            Close
          </button>
          <button
            className="hover:bg-[var(--cc-bg-hover)] w-full cursor-pointer px-3 py-1.5 text-left"
            onClick={() => fs.closeOtherTabs(ctxMenu.path)}
          >
            Close Others
          </button>
          <button
