...
export const fsCommandEntries: Array<[string, CommandDef]> = [
  [
    'ls',
    createFsCommand('list files', (ctx, vfs) => {
  ...
  [
    'cd',
    createFsCommand('change directory', (ctx, vfs) => {
      const targetArg = getSingleCommandArg(ctx.command);
  ...
  [
    'mkdir',
    createFsCommand('create directory', (ctx, vfs) => {
      const targetArg = getSingleCommandArg(ctx.command);
      if (!targetArg) {
  ...
  [
    'touch',
    createFsCommand('create file', (ctx, vfs) => {
      const targetArg = getSingleCommandArg(ctx.command);
      if (!targetArg) {
  ...
  [
    'rm',
    createFsCommand('remove file or dir', (ctx, vfs) => {
  ...
  [
    'mv',
    createFsCommand('move / rename', (ctx, vfs) => {
  ...
  [
    'cp',
    createFsCommand('copy file', (ctx, vfs) => {
      const pairArgs = getPairCommandArgs(ctx.command);
      if (!pairArgs) {
  ...
  [
    'cat',
    createFsCommand('print file contents', (ctx, vfs) => {
      const targetArg = getSingleCommandArg(ctx.command);
      if (!targetArg) {
